...
export default function NotificationsPopover() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/facultyname').then((r) => {
      setfacName(r.data);
    });
  }, []);
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/notifications').then((r) => {
  ...
  const handleEditClick = (row) => {
    axios.get(`http://localhost:8080/api/notifications/${row.row._id}`).then((r) => {
      setEdit(row.row._id);
      setdata({
        name: r.data.name,
  ...
  const handleDeleteClick = (row) => {
    if (window.confirm('Are you sure you want to delete?')) {
      axios.delete(`http://localhost:8080/api/notifications/${row.row._id}`).then(() => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:8080/api/notifications', data).then((r) => {
      setEdit(r.data._id);
    });
    setdata({
      name: '',
