...
const Session = (props) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/session').then((r) => {
      setshow(r.data);
    });
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (data.session) {
        ...
        const response = await axios.put(`http://localhost:8080/api/session/${data._id}`, data);
        toast.success('Data updated successfully');
      } else {
        // If data.session is empty, make a POST request
        const response = await axios.post('http://localhost:8080/api/session', data);
