...
import { Button, Card, Container, Grid, CardActionArea, Typography, Stack, styled } from '@mui/material';
...
const Syllabus = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:8080/api/subject')
      .then((r) => {
        let modifiedData = [];
  ...
  const handleSubmit = (e) => {
    ...
    e.preventDefault();
    axios.post('http://localhost:8080/api/subassignment', formdata).then((r) => {
      setEdit(r.data._id);
      refresh();
    });
  };
  ...
  const handleDriveSubmit = (e, index) => {
    e.preventDefault();
    axios.post('http://localhost:8080/api/subdrive', Drivedata).then((r) => {
      DsetEdit(r.data._id);
      Drefresh();
    });
  };
  ...
  const handleNoticeSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:8080/api/subnotice', Noticedata).then((r) => {
      NsetEdit(r.data._id);
      Nrefresh();
    });
  };
  ...
  const updateData = (e) => {
    axios.put(`http://localhost:8080/api/subassignment/${edit}`, data).then((r) => {
      console.log(r.data);
      setEdit(r.data._id);
      refresh();
    });
  ...
  const DupdateData = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:8080/api/subdrive/${Dedit}`, Drivedata).then((r) => {
      DsetEdit(r.data._id);
      Drefresh();
    });
  };
  ...
  const NupdateData = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:8080/api/subnotice/${Nedit}`, Noticedata).then((r) => {
      console.log(r.data);
      NsetEdit(r.data._id);
      Nrefresh();
    });
  ...
  const handleDeleteClick = (row) => {
    if (window.confirm('Are you sure you want to delete?')) {
      axios.delete(`http://localhost:8080/api/subassignment/${row.row._id}`).then(() => {
  ...
  const DhandleDeleteClick = (row) => {
    if (window.confirm('Are you sure you want to delete?')) {
      axios.delete(`http://localhost:8080/api/subdrive/${row.row._id}`).then(() => {
  ...
  const NhandleDeleteClick = (row) => {
    if (window.confirm('Are you sure you want to delete?')) {
      axios.delete(`http://localhost:8080/api/subnotice/${row.row._id}`).then(() => {
  ...
  const handleEditClick = (row) => {
    axios.get(`http://localhost:8080/api/subassignment/${row.row._id}`).then((r) => {
      setEdit(row.row._id);
      setData({
  ...
  const DhandleEditClick = (row) => {
    axios.get(`http://localhost:8080/api/subdrive/${row.row._id}`).then((r) => {
  ...
  const NhandleEditClick = (row) => {
    axios.get(`http://localhost:8080/api/subnotice/${row.row._id}`).then((r) => {
  ...
  const refresh = () => {
    axios.get(`http://localhost:8080/api/subassignment`).then((r) => {
      const data = r.data;
  ...
  const Drefresh = () => {
    axios.get(`http://localhost:8080/api/subdrive`).then((r) => {
      const data = r.data;
      const subjectName = subData[activeIndex].subject;
      const filteredData = data
        .filter((value) => value.subject === subjectName)
  ...
  const Nrefresh = () => {
    axios.get(`http://localhost:8080/api/subnotice`).then((r) => {
      const data = r.data;
      const subjectName = subData[activeIndex].subject;
      const filteredData = data
        .filter((value) => value.subject === subjectName)
